      ...
      case "SingleCall":
        if (obj.type == "SCallMCfm") {
          ...
          if (confirmRet) {
            ...
            calledNO=jsonObj.pcDn;
            calledType= callType;
            excuteFucOfPlugin("AcceptCall",[scallHandler]);
          } else {
            CloseSingleCall();
          }
      ...
      case "Video":
        ...
        if (obj.type == "SVideoMtInd") {
          ...
          if (ret) {
            ...
            excuteFucOfPlugin("CloseVideo", [hUserCall]);
          }
        }
        break;
      case "Msg":
          ...
          switch (obj.msg.method) {
            case "VerifyWebSoket":
              ...
              if (obj.msg.result === "34c61afbc8b6470f9b349a1e1c2986be") {
                ...
                verifyResult = true;
                excuteFucOfPlugin("GetSurfaceModel",[]);
                ...
                try {
                  // var name =  '2100100';
                  // var pwd = '2100100';
                  // var name ='15302768668'
                  // var pwd ='15302768668'

                  var name = localStorage.getItem('user_info_phone')
                  var pwd = localStorage.getItem('user_info_phone')

                  var ip = '58.248.254.109';
                  if (BrowserIsIE()) {
              ...
              break;
            case "GetSurfaceModel":
              let surface= obj.msg.extent
              
              $("#callSurfaceID").prop("checked", surface.CALL_SURFACE);
              $("#msgSurfaceID").prop("checked",surface.MSG_SURFACE);
              $("#msgSurfaceDiv").css("display",surface.MSG_SURFACE?"block":"none")
              $("#msgNoSurfaceDiv").css("display",surface.MSG_SURFACE?"none":"block")
              break;
            case "SetSurfaceModel":
              if (obj.msg.result) {
                alert("界面模式修改成功！")
                let msg_surface = document.getElementById("msgSurfaceID").checked;
                $("#msgSurfaceDiv").css("display",msg_surface?"block":"none")
                $("#msgNoSurfaceDiv").css("display",msg_surface?"none":"block")
              }else{
                alert("界面模式修改失败！")
              }
              break;
